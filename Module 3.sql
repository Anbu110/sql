...
ALTER TABLE canteen
DROP CONSTRAINT CK__canteen__salary__6A30C649;

alter table canteen 
drop column salary -- drop a cloumn that contains constraint of check

ALTER TABLE canteen
DROP CONSTRAINT CK__canteen__salary__6A30C649; -- droping a constraint
...
alter column employeefirstname char(50) not null
...
CREATE FUNCTION fn_GetEmployeeFullName1
(
...
SELECT dbo.fn_GetEmployeeFullName1('John','Smith') as FullName
SELECT dbo.fn_GetEmployeeFullName1('Dinesh','Kumar') as FullName

CREATE  FUNCTION fn_AddNumbers1
(
	@number1 int
	, @number2 int
)
...
select dbo.fn_AddNumbers1(1,2)
select dbo.fn_AddNumbers1(10,15)
select dbo.fn_AddNumbers1(11,null)
...
WHERE dbo.fn_GetEmployeeFullName(EmployeeFirstName, EmployeeLastName) = 'Dinesh kumar'
...
CREATE FUNCTION fn_GetEmployeeDetails1
(
	@Address varchar(100)
)
RETURNS TABLE
AS
	RETURN (SELECT dbo.fn_GetEmployeeFullName1(EmployeeFirstName,EmployeeLastName)as FullName
				, Salary
				, Address
				, EmployeeDOB
			From Employee as emp
...
select * from dbo.fn_GetEmployeeDetails1('US')
...
CREATE OR ALTER FUNCTION fn_GetEmployeeDetailsML1
(	
	@Employee_ID int
)
RETURNS @Emp TABLE 
...
BEGIN
	INSERT INTO @Emp
	SELECT emp.EmployeeID, dbo.fn_GetEmployeeFullName1(EmployeeFirstName,EmployeeLastName),emp.Salary
	FROM Employee as emp
	...
	UPDATE @Emp
	SET Salary = 5000
	WHERE Employee_Id IN (2,5)

	DELETE FROM @Emp
	WHERE Employee_Id IN (6,3)
...
SELECT *
	,CASE WHEN EmployeeId % 21=0 THEN 'Odd' ELSE 'Even' END AS EvenOROdd
FROM Employee
...
SELECT *,	
	CASE 
		WHEN salary >=0 and Salary<=1000 THEN '0 to 1000'
